...
namespace BetaMart
{
    ...
    {
        OleDbConnection koneksi = new OleDbConnection(@"Provider = Microsoft.ACE.OLEDB.12.0; Data Source = C:\UKK KABEH\DatabaseBetaMart.accdb");
        public FormUtama()
        {
            InitializeComponent();
        }
        ...
        {
            koneksi.Open();
            string perintah = "select * from barang where kode='" + tbcari.Text + "'";
            OleDbDataAdapter da = new OleDbDataAdapter(perintah, koneksi);
            DataTable dt = new DataTable();
            da.Fill(dt);
            dataGridView1.DataSource = dt;
